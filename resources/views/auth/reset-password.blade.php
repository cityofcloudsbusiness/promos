        ...
        <div class="w-full max-w-md bg-black/40 backdrop-blur-2xl border border-white/10 p-10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
            ...
            <div class="text-center mb-8">
                <h2 class="text-white font-extralight uppercase tracking-[0.6em] text-2xl">
                    RESET<span class="font-bold text-pink-500"> KEY</span>
                </h2>
                <p class="text-[10px] text-pink-400/60 tracking-[0.3em] uppercase mt-2">Protocolo de Recuperação de Acesso</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.store') }}" class="space-y-6">
                ...
                <div class="relative">
                    <input id="email" type="email" name="email" :value="old('email', $request->email)" required autofocus 
                        ...
                        placeholder="E-MAIL DE IDENTIFICAÇÃO">
                    <x-input-error :messages="$errors->get('email')" class="mt-1 text-pink-500 text-[10px] uppercase" />
                </div>

                <div class="relative">
                    <input id="password" type="password" name="password" required 
                        class="w-full bg-transparent border-0 border-b border-white/20 text-white focus:ring-0 focus:border-purple-500 transition-all px-0 py-3 placeholder:text-white/20 uppercase text-xs tracking-widest"
                ...
                <div class="relative">
                    ...
                    <x-input-error :messages="$errors->get('password_confirmation')" class="mt-1 text-purple-500 text-[10px] uppercase" />
                </div>
