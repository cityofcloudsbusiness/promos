...
<head>
    ...
    <title>Entrar · City of Clouds EscolaOnline</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="h-full bg-indigo-950 antialiased overflow-hidden">
    ...
    {{-- Glow top-right --}}
    <div class="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full pointer-events-none"
    ...
    <span class="absolute top-6 right-6 font-mono text-[9px] text-indigo-700 select-none">X:FF Y:00</span>
    <span class="absolute bottom-6 left-6 font-mono text-[9px] text-indigo-700 select-none">X:00 Y:FF</span>
    <span class="absolute bottom-6 right-6 font-mono text-[9px] text-indigo-700 select-none">X:FF Y:FF</span>
    ...
    <div class="relative h-full flex items-center justify-center px-4">
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-2xl font-bold text-white tracking-tight">Acessar a plataforma</h1>
                <p class="text-indigo-300 text-sm mt-1.5">Bem-vindo(a) de volta à EscolaOnline</p>
            </div>
            ...
            <div class="bg-white/[0.04] backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl shadow-black/30">
                ...
                {{-- Session status --}}
                @if(session('status'))
                    <div class="mb-5 px-4 py-3 bg-emerald-900/40 border border-emerald-500/30 text-emerald-300 rounded-xl text-sm">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <label for="password" class="block text-sm font-medium text-indigo-200">
                                Senha
                            </label>
                            @if(Route::has('password.request'))
                                <a href="{{ route('password.request') }}"
                                   class="text-xs text-indigo-400 hover:text-indigo-200 transition">
                                    Esqueceu a senha?
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <label for="remember_me" class="text-sm text-indigo-300 select-none">
                            Manter conectado
                        </label>
                    </div>
                ...
                <a href="{{ route('register') }}"
                   class="flex items-center justify-center gap-2 w-full py-3 px-6
                    ...
                    Ainda não tem conta? <span class="font-semibold text-indigo-300">Criar conta grátis</span>
                </a>
            </div>
